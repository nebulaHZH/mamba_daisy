...
class SequenceWindowDataset(Dataset):
    ...
    def _fit_scaler(self):
        ...
        for record in tqdm(self.file_records, desc="Scaler"):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(window.copy()), record.label_id
...
def main():
    ...
    train_records, val_records, test_records = split_file_records(file_records, test_size=0.2, val_size=0.1)
    ...
    print(f"Input shape: (batch, {config['window_size']}, {input_channels})")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if patience_counter >= patience:
            print(f"\nEarly stopping triggered! Val acc did not improve for {patience} epochs")
            break

    plot_training_history(history, save_path=config["history_path"])
